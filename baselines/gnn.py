...
class GraphSAGEConv(torch.nn.Module):
    def __init__(self, input_dim, rank, order, args):
        ...
        self.layers = torch.nn.ModuleList([dgl.nn.pytorch.SAGEConv(rank if i != 0 else input_dim, rank, aggregator_type='gcn') for i in range(order)])
    ...
    def forward(self, graph, features):
        g, feats = graph, graph.ndata['features'].float()
        for i, (layer, norm, act) in enumerate(zip(self.layers, self.norms, self.acts)):
            feats = layer(g, feats)
            feats = norm(feats)
            feats = act(feats)
            feats = self.dropout(feats)
        batch_sizes = torch.as_tensor(g.batch_num_nodes())  # 每个图的节点数
        first_nodes_idx = torch.cumsum(torch.cat((torch.tensor([0]), batch_sizes[:-1])), dim=0)  # 使用torch.cat来连接Tensor
        # 获取每个图的首个节点的特征
